...
- **Mode:** Phase 2 of Telegram→CC bridge **shipped on `main`** (`378c854`, 2026-04-19 — squash of 14 feature commits). All implementation + tests done; live E2E smoke is pending on the owner's side post-deploy and is not blocking any code work.
...
- **Live E2E pending on owner's side (post-deploy):** after Railway + Vercel deploys go green, mint a `telegram-bridge` API token in Settings → API Tokens, paste into `telegram_bot/.env` as `HUB_API_TOKEN`, set `HUB_API_URL=https://backend-api-production-1967.up.railway.app`, configure `telegram_user_id` + PIN in Settings → Telegram → Telegram Bridge, then run AC-3/4/6/7/9 from PRD (`/unlock` wrong/correct → `git push` locked/unlocked → non-whitelisted TG account silent drop → `/new` resets context → `Notes/Personal/**` denied in both profiles). If anything breaks, fix on a fresh `fix/telegram-bridge-*` branch.
